...
```{r create_dirs}
dir.create(file.path("../troubleshoot/", "MAGMA")) # create MAGMA directory. 

dir.create(file.path("../troubleshoot/MAGMA", "analysis")) # create analysis subdir. 

dir.create(file.path("../troubleshoot/MAGMA", "data")) # create data subdir. 

dir.create(file.path("../troubleshoot/MAGMA", "output")) # create output subdir. 
```
...
```{r metadata_export}
write_tsv(x = MAGMA_meta, path = "../troubleshoot/MAGMA/data/metadata.txt", na = "NA", append = FALSE, col_names = TRUE, quote_escape = "double")
```
...
I'm going to load in the harvest data for the 2018 season so I can check otut he harvest by district and by stat week.  The ultimate goal is to create a summary table of harvest at each district - subdistrict for each statweek.   For example: 
...
It looks like we have a bit of harvest after week 34. Since I only have collection information to SW34, I am going to end the analysis here. Any age-comps after this will be projections. 

```{r}
MAGMA_harv <- harvest_raw %>%
  select(c(Year, `Stat Area`, `Stat Week`, `Number Of Animals (sum)`)) %>%  # keeping only the important columns.
  mutate(DISTRICT = str_sub(harvest_raw$`Stat Area`, start = 1, end = 3),
         SUBDISTRICT = "00") %>%
  rename(STAT_WEEK = `Stat Week`,
         YEAR = Year,
         HARVEST = `Number Of Animals (sum)`) %>%
  select(YEAR, DISTRICT, SUBDISTRICT, STAT_WEEK, HARVEST,-`Stat Area`)  %>%
  filter(STAT_WEEK < 35) %>% # only considering SW 24-34 due to available data. 
  group_by(YEAR, DISTRICT, SUBDISTRICT, STAT_WEEK, STAT_WEEK) %>% 
  summarize(HARVEST = sum(HARVEST)) %>% 
  ungroup()
``` 

```{r harvest_export}
write_tsv(x = MAGMA_harv, path = "../troubleshoot/MAGMA/data/harvest.txt", na = "NA", append = FALSE, col_names = TRUE, quote_escape = "double")
```
...
group_names <- group_names %>% 
  rename(D115 = `unique(groups4_pops239$GROUPS)`)
...
```{r export_txt}
write_tsv(x = group_names, path = "../troubleshoot/MAGMA/data/group_names.txt", na = "NA", append = FALSE, col_names = TRUE, quote_escape = "double") # export object group_names

write_tsv(x = groups, path = "../troubleshoot/MAGMA/data/groups.txt", na = "NA", append = FALSE, col_names = TRUE, quote_escape = "double")
```

 # Note- There are no marked fish, so do I need to add the last few groups to the groups.txt file?? I found them in Serena's old files (2016-2017)...
BURN	1
MAIN	1
SPEE	1
SWEE	1
